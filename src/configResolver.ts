...
export type ConfigResult = { ok: true; config: RunConfig } | { ok: false; error: ConfigValidationError };
...
export function parseConfigToml(toml: string, source: "global" | "project" = "global"): { ok: true; overlay: Overlay } | { ok: false; error: ConfigValidationError } {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!assignment) {
      return { ok: false, error: validationError("invalid_toml", `Invalid TOML syntax in ${source} config at line ${lineNumber}. Expected [run] or key = positive_integer.`, `line ${lineNumber}`) };
    }
    if (table !== "run") {
      return { ok: false, error: validationError("invalid_toml", `Config key ${assignment[1]} must be inside a [run] table.`, assignment[1] ?? `line ${lineNumber}`) };
    }
    ...
    if (!Number.isSafeInteger(value) || value <= 0) {
      return { ok: false, error: validationError("invalid_config_value", `[run].${tomlKey} must be a positive safe integer.`, `run.${tomlKey}`) };
    }
...
function normalizePerRun(perRun: Record<string, unknown> | undefined): { ok: true; perRun: PerRun } | { ok: false; error: ConfigValidationError } {
  const out: PerRun = {};
  if (!perRun) return { ok: true, perRun: out };

  for (const [field, raw] of Object.entries(perRun)) {
    if (!CONFIG_FIELDS.has(field as keyof RunConfig)) continue;
    if (!Number.isSafeInteger(raw) || (raw as number) <= 0) {
      return { ok: false, error: validationError("invalid_config_value", `${field} must be a positive safe integer.`, field) };
    }
...
export async function resolveRunConfig(args: {
  cwd?: string;
  homeDir?: string;
  ...
  const globalConfigPath = args.globalConfigPath ?? join(homeDir, ".pi", "lambda-rlm", "config.toml");
  const projectConfigPath = args.projectConfigPath ?? join(cwd, ".pi", "lambda-rlm", "config.toml");

  let config = DEFAULT_RUN_CONFIG;
  for (const [source, path] of [["global", globalConfigPath], ["project", projectConfigPath]] as const) {
  ...
  for (const [field, value] of Object.entries(perRun.perRun) as Array<[keyof RunConfig, number]>) {
    if (value > config[field]) {
      return {
        ok: false,
        error: validationError("per_run_limit_loosened", `${field}=${value} would loosen the resolved limit ${config[field]}. Per-run options may only tighten limits.`, field),
      };
    }
