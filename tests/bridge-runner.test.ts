...
describe("Python NDJSON bridge runner", () => {
  it("starts the Python bridge, answers one request-identified synthetic model callback, and returns exactly one final result", async () => {
    const result = await runSyntheticBridge({
      bridgePath,
      runId: "run-test-1",
      question: "What is this file about?",
      contextPath: "context.txt",
      fakeModelResponse: "synthetic model answer",
    });
