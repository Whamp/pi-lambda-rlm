...
import { promptKeys, resolvePromptBundle } from "../src/promptResolver.js";
...
describe("prompt overlay resolver", () => {
  it("loads private built-in defaults for the full prompt surface when no overlays exist", async () => {
    ...
    if (!result.ok) return;
    expect(Object.keys(result.bundle.prompts).sort()).toEqual(promptKeys().sort());
    for (const key of promptKeys()) expect(result.bundle.prompts[key]?.source.layer).toBe("built_in");
  ...
  it("applies sparse global and project overlays file-by-file with project precedence and inheritance", async () => {
    ...
    await writeOverlay(cwd, ".pi/lambda-rlm/prompts/filters/relevance.md", "PROJECT FILTER <<query>> / <<preview>>");
    ...
    expect(result.bundle.prompts["filters/relevance.md"]?.template).toBe("PROJECT FILTER <<query>> / <<preview>>");
  ...
  it("fails before execution on unknown placeholders and missing required placeholders", async () => {
    ...
    await writeOverlay(cwdMissing, ".pi/lambda-rlm/prompts/reducers/select-relevant.md", "Bad <<parts>> only");

    const missing = await resolvePromptBundle({ homeDir, cwd: cwdMissing });
    expect(missing).toMatchObject({ ok: false, error: { type: "validation", code: "missing_required_prompt_placeholder", field: "reducers/select-relevant.md" } });
  });
